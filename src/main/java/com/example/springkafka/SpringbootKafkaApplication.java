...
public class SpringbootKafkaApplication {
	...
	public static class MessageProducer {
		...
		public void sendMessage(String message) {
			...
			// MessageBuilder.withPayload(message).setHeader(KafkaHeaders.MESSAGE_KEY, key)
			// .setHeader(KafkaHeaders.TOPIC, topic).build();
			ListenableFuture<SendResult<String, String>> send = kafkaTemplate.send("ddd", message);

			send.addCallback(new ListenableFutureCallback<SendResult<String, String>>() {

				@Override
				public void onSuccess(SendResult<String, String> result) {
					System.out.println("#### Inside onSuccess,partition=> " + result.getRecordMetadata().partition() + " Offset => "+result.getRecordMetadata().offset() + " Topic => " + result.getRecordMetadata().timestamp());
...
//		@KafkaListener(topics = "${message.topic.name}", group = "foo", containerFactory = "fooKafkaListenerContainerFactory")
		@KafkaListener(topics = "ddd", group = "foo", containerFactory = "fooKafkaListenerContainerFactory")
		public void listenGroupFoo(ConsumerRecord<?, ?> record) {
//			System.out.println(Thread.currentThread().getId() + "   Received Messasge in group 'foo': " + record);
			System.out.println("Received " + record.value() +  " from partition=> "+record.partition() +" offset => "+ record.offset());
			// latch.countDown();
		}

		// @KafkaListener(topics = "${message.topic.name}", group = "bar",
		...
		// System.out.println("Received Messasge: " + message + " from partition: " +
		// partition);
		// latch.countDown();
		// }

		// @KafkaListener(topicPartitions = @TopicPartition(topic =
		// "${partitioned.topic.name}", partitions = { "0",
		// "3" }))
		...
		// @KafkaListener(topics = "${filtered.topic.name}", containerFactory =
